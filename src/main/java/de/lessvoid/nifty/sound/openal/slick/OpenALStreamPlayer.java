...
public class OpenALStreamPlayer {
	...
	/**
	 * Check if the playback is complete. Note this will never return true if we're
	 * looping
	 *
		...
		try {
			...
			if (count != -1) {
				...
				try {
					...
					log.warning("Failed to loop buffer: " + bufferId + " " + format + " " + count + " "
							+ audio.getRate() + e.toString());
					return false;
				}
			} else {
				if (loop) {
